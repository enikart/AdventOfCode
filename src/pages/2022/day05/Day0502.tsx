...
class Day0502 extends React.Component<any, any> {
    state: {
        answer: string
    }
    ...
    makeMove(map: string[][], move: number, from: number, to: number): string[][] {
        let cratesMoving: string[] = [];
        for (let i = 0; i < move; i++) {
            let crateMoving = map[from - 1].shift();
            if (crateMoving) {
                cratesMoving.unshift(crateMoving);
            }
        }

        cratesMoving.forEach(crateMoving => {
    ...
    render() {
        return (
            <div>
                <h2>Exercise 05 Part 2</h2>
                <ExerciseComponent yearNumber={2022} exerciseNumber={5} answer={this.state.answer} rightAnswer={"CNSCZWLVT"} exec={this.execute}/>
            </div>
        );
    }
}

export default Day0502;
